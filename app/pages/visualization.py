...
def show_page():
    ...
    if st.session_state["df"] is not None or st.session_state.get("df_merged") is not None or st.session_state.get("db_path"):
        ...
        if df is not None and not df.empty:
            ...
            # Vérifier si les colonnes numériques et catégorielles existent
            if not num_columns:
                st.warning("Aucune colonne numérique détectée dans les données.")
                return
            
            if not cat_columns:
                st.warning("Aucune colonne catégorielle détectée dans les données.")
                cat_columns = ["Aucune colonne"]
            ...
            with st.sidebar.expander("⚙️ Configuration du graphique 1 (Tendances)"):
                 ...
                 bar_type = st.selectbox("Type de graphique", graph_types_1, index=graph_types_1.index(bar_type) if bar_type in graph_types_1 else 0, key="bar_type")
    
    # Sélecteurs d'axes l'un sous l'autre
                 bar_x = st.selectbox("Axe X", all_columns, index=all_columns.index(bar_x) if bar_x in all_columns else 0, key="bar_x")
                 bar_y = st.selectbox("Axe Y", num_columns, index=num_columns.index(bar_y) if bar_y in num_columns else 0, key="bar_y")
                
                
            
            with st.sidebar.expander("⚙️ Configuration du graphique 2 (Répartition)"):
                  graph_types_2 = ["pie", "bar", "funnel", "treemap"]
                  pie_type = st.selectbox("Type de graphique", graph_types_2, index=graph_types_2.index(pie_type) if pie_type in graph_types_2 else 0, key="pie_type")
    
                  pie_col = st.selectbox("Colonne de catégories", cat_columns, index=cat_columns.index(pie_col) if pie_col in cat_columns else 0, key="pie_col")
            ...
            with st.sidebar.expander("⚙️ Configuration du graphique 3 (Évolution)"):
                 ...
                 line_type = st.selectbox("Type de graphique", graph_types_3, index=graph_types_3.index(line_type) if line_type in graph_types_3 else 0, key="line_type")
    
    # Sélecteurs d'axes l'un sous l'autre
                 line_x = st.selectbox("Axe X", all_columns, index=all_columns.index(line_x) if line_x in all_columns else 0, key="line_x")
                 line_y = st.multiselect("Métriques", num_columns, default=line_y if isinstance(line_y, list) and all(y in num_columns for y in line_y) else [num_columns[0]], key="line_y")
            
            with st.sidebar.expander("⚙️ Configuration du graphique 4 (Comparaison)"):
                 graph_types_4 = ["scatter", "line", "bar", "area"]
                 bubble_type = st.selectbox("Type de graphique", graph_types_4, index=graph_types_4.index(bubble_type) if bubble_type in graph_types_4 else 0, key="bubble_type")
    
    # Sélecteurs d'axes l'un sous l'autre
                 bubble_x = st.selectbox("Axe X", num_columns, index=num_columns.index(bubble_x) if bubble_x in num_columns else 0, key="bubble_x")
                 bubble_y = st.selectbox("Axe Y", num_columns, index=num_columns.index(bubble_y) if bubble_y in num_columns else (1 if len(num_columns) > 1 else 0), key="bubble_y")
    
                 if bubble_type == "scatter":
        # Sélecteurs de taille et couleur l'un sous l'autre
                                bubble_size = st.selectbox("Taille", num_columns, index=num_columns.index(bubble_size) if bubble_size in num_columns else 0, key="bubble_size")
                                bubble_color = st.selectbox("Couleur", cat_columns, index=cat_columns.index(bubble_color) if bubble_color in cat_columns else 0, key="bubble_color")
            
            with st.sidebar.expander("⚙️ Configuration du graphique 5 (Distribution)"):
                graph_types_5 = ["box", "violin", "histogram", "bar"]
                box_type = st.selectbox("Type de graphique", graph_types_5, index=graph_types_5.index(box_type) if box_type in graph_types_5 else 0, key="box_type")
                
                
                
                boxplot_col = st.selectbox("Axe X", num_columns, index=num_columns.index(boxplot_col) if boxplot_col in num_columns else 0, key="boxplot_col")
            ...
            with col3:
                unique_count = 0
                if len(cat_columns) > 0 and cat_columns[0] != "Aucune colonne":
            ...
            with col1:
                ...
                if len(num_columns) > 0 and len(cat_columns) > 0 and bar_x in df.columns and bar_y in df.columns:
                    ...
                    if bar_type == "bar":
                        ...
                        fig = px.line(df, x=bar_x, y=bar_y, 
                                      color_discrete_sequence=[color_primary],
                                      template="plotly_white")
                        
                        if show_second_year and second_y and second_y in df.columns:
                            fig.add_scatter(x=df[bar_x], y=df[second_y], 
                                           mode='lines+markers',
                                           name=second_y, marker_color=color_secondary)
                    elif bar_type == "area":
                        fig = px.area(df, x=bar_x, y=bar_y, 
                                      color_discrete_sequence=[color_primary],
                                      template="plotly_white")
                        
                        if show_second_year and second_y and second_y in df.columns:
                            temp_df = df.copy()
                            fig = px.area(temp_df, x=bar_x, y=[bar_y, second_y], 
                                          color_discrete_sequence=[color_primary, color_secondary],
                                          template="plotly_white")
                    else:  # scatter
                        fig = px.scatter(df, x=bar_x, y=bar_y, 
                                         color_discrete_sequence=[color_primary],
                                         template="plotly_white")
